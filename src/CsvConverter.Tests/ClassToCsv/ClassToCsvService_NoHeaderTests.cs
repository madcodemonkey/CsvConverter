...
{
    [TestClass]
    public class ClassToCsvService_NoHeaderTests
    {   
        [DataTestMethod]
            ...
            int order, int age, string name, string expectedOrder, string expectedAge, string expectedName)
        {
            ...
            var classUnderTest = new ClassToCsvService<ClassToCsvServiceNoHeaderData1>(rowWriterMock);
            classUnderTest.Configuration.HasHeaderRow = false;

            var data = new ClassToCsvServiceNoHeaderData1() { Order = order, Age = age, Name = name };
    ...
    internal class ClassToCsvServiceNoHeaderData1
    {
